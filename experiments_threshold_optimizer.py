...
to_xgb = ThresholdOptimizer(estimator=gs_xgb.best_estimator_, constraints="demographic_parity", objective="accuracy_score")
to_xgb.fit(X, y, sensitive_features=pd.DataFrame(X, columns=protected_attributes))
...
xgb_pred = to_xgb.predict(X_test, sensitive_features=pd.DataFrame(X, columns=protected_attributes))
